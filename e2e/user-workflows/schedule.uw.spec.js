...
describe('Schedule', () => {
  ...
  it('Can remove a schedule', async () => {
    ...
    const allScheduleCalls = await getRequestCount('/app/v1/mowers/190415542-190332911/timers', 'GET')

    await SchedulePage.goTo()
    expect(await SchedulePage.isVisible()).toBeTruthy()

    await verifyUrlCalled('/app/v1/mowers/190415542-190332911/timers', 'GET', allScheduleCalls + 1)

    await SchedulePage.tapOnEditButton()
    expect(await SchedulePage.isEditModeVisible()).toBeTruthy()

    await verifyUrlCalled('/app/v1/mowers/190415542-190332911/timers', 'GET', allScheduleCalls + 2)

    expect(await SchedulePage.getSchedulesCount()).toBe(1)
    await SchedulePage.tapOnDeleteButton()
    await SchedulePage.tapOnSaveButton()

    await verifyUrlCalled('/app/v1/mowers/190415542-190332911/timers', 'PUT', allScheduleCalls + 1, { timers: [] })
    await verifyUrlCalled('/app/v1/mowers/190415542-190332911/timers', 'GET', allScheduleCalls + 3)
  })
})