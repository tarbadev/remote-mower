...
export const mainBindings = function (ipcMain, browserWindow, fs, distPath) {
  ipcMain.on(readFileRequest, (IpcMainEvent, args) => {
    ...
    fs.readFile(`${distPath}${args.filename}`, callback)
  })
...
class Backend {
  ...
  init(services, backendOptions, i18nextOptions) {
    if (typeof window.api.i18nextElectronBackend === 'undefined') {
      throw Error(
        '\'window.api.i18nextElectronBackend\' is not defined! Be sure you are setting up your BrowserWindow\'s preload script properly!')
    }
  ...
  write(writeQueue) {
    ...
    for (let i = 0; i < toWork.length; i++) {
      const anonymous = function (error, data) {
        if (error) {
          console.error(`${this.rendererLog} encountered error when trying to read file '${data.filename}' before writing missing translation ('${data.key}'/'${data.fallbackValue}') to file. Please resolve this error so missing translation values can be written to file. Error: '${error}'.`)
          return
        }
        ...
        if (debug) {
          console.log(`${this.rendererLog} requesting the missing key '${data.key}' be written to file '${data.filename}'.`)
        }
